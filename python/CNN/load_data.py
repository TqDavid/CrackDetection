...
#                    b_show = Image.fromarray(block.astype(np.uint8))
#                    display(b_show)
#                    l_show = Image.fromarray(block_label.astype(np.uint8))
#                    display(l_show)
                    dataset.append(block)
                    num_block += 1
#        print("Complete departing image %d" % num_image)
...
# In[2]:pickle
test_datasets_pos, test_label_pos = depart_pos(os.path.join(path, 'test'))
test_datasets_neg, test_label_neg = depart_neg(os.path.join(path, 'test'))
valid_datasets_pos, valid_label_pos = depart_pos(os.path.join(path, 'validation'))
valid_datasets_neg, valid_label_neg = depart_neg(os.path.join(path, 'validation'))
train_datasets_pos, train_label_pos = depart_pos(os.path.join(path, 'train'))
train_datasets_neg, train_label_neg = depart_neg(os.path.join(path, 'train'))
...
# 显示图片
#length = len(test_datasets_pos)
#for i in range(length):
#    im = Image.fromarray(test_datasets_pos[i].astype(np.uint8))
#    display(im)
#    label = Image.fromarray(test_label_neg[0].astype(np.uint8))
