...
def accuracy(predictions, labels):
    ...
    compare = np.multiply(predictions == labels, 1)
    return (100.0 * np.sum(compare) / predictions.shape[0] /predictions.shape[1])
...
with tf.Session(graph=graph) as session:
    ...
    img_pad = np.pad(img, half_struct, 'constant', constant_values=0)
