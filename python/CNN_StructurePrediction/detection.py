...
#    session.graph.finalize()
    for num_pic in range(1,119):
        ...
        outpad = np.pad(output, half_block, 'constant', constant_values=0)
        for i in range(3):
        ...
        for x in range(width):
            for y in range(height):
                ...
                outpad[x:x + 2*half_struct + 1, y:y + 2*half_struct + 1] = prediction
        output = outpad[half_block:width+half_block, half_block:height+half_block, :]
        output = output / tf.reduce_max(img) * 255
